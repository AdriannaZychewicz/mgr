...
r_dir = '/home/klichota/mgr-ada/mgr/tracks/wave'
...
for root, sub, files in os.walk(r_dir):
    ...
    for f in files:
        ...
        for delta_t in intervalos:
            ...
            #time_rms = np.arange(len(x1_part_rms)) * float(1) / (float(w[0]))
            ...
            # Signal
            pp.plot(x * float(1) / float(w[0]), x1, color='c', label='Time Signal')
            # Abs value of signal
            # pp.plot(x*float(1)/float(w[0]),abs(x1), color='y',label='Absolute value of Time Signal')

            # Low pass filter envelope #

            #pp.plot(time_part, filtered_aver_vs_ped_simp, linestyle='--', color='b', label='Low-pass Filter envelope',

            #       linewidth=3)

            # Rms envelope #
            #pp.plot(time_rms + 500 / float(w[0]), x1_part_rms, linestyle='-', color='k', label='RMS aproach envelope',

            #        linewidth=1)

            # Our implementation #

            # pp.plot(time_part,aver,color='k',label='Second step for envelope',linewidth=1)# Pre-envelope
            ...
            pp.xlim([delta_t * float(1) / float(w[0]), (delta_t + step) * float(1) / float(w[0]) + 0.001])
            pp.xticks(
                np.arange(delta_t * float(1) / float(w[0]), (delta_t + step) * float(1) / float(w[0]) + 0.001, 0.1),
                fontsize=12)
            ...
            pp.legend(fontsize='x-small', loc=3)
            ...
            pp.specgram(x1, NFFT=int(w[0] * 0.01), window=scipy.signal.tukey(int(w[0] * 0.01)), Fs=w[0],
                        noverlap=int(w[0] * 0.005), cmap='jet')
            ...
            pp.legend(fontsize='x-small')
            # pp.tick_params( axis='x', labelbottom='off')

            figname = "%s.png" % (str(base) + '_signal_zoom_' + str(delta_t * float(1) / float(w[0])))
            pp.savefig(os.path.join(base, figname), dpi=50)
            pp.close('all')

            ###############################################################
            # save in plot file txt with data if necesary

            # f_out = open('/home/klichota/mgr-ada/mgr/plots/data_txt/%s.txt' %(str(base)+'_'+str(delta_t*float(1)/float(w[0]))), 'w')
