...
from calculate_time_feature import get_signal
...
if __name__ == "__main__":
    ...
    _max, _min = peakdetect(y_axis=signal, x_axis=None, lookahead=700, delta=0.30)
    # print _max
    xm = [p[0] for p in _max]
    ym = [p[1] for p in _max]
    xn = [p[0] for p in _min]
    ...
    # print min(xm)
    print
    sorted_tuples = sorted(zip(xm, ym), key=lambda z: z[1])
    print  map(list, zip(*sorted_tuples))
    xm_min = sorted_tuples[0][0:10]
    ym_min = sorted_tuples[1][0:10]
    # cisza
    from pydub import AudioSegment, silence
    # myaudio = intro = AudioSegment.from_wav("a-z-vowels.wav")
    # silence = silence.detect_silence(signal, min_silence_len=1000, silence_thresh=-16)
    # silence = [((start / 1000), (stop / 1000)) for start, stop in silence]  # convert to sec
    #print silence
    # y_silent = [a for a in signal if (300 > a > -300)]
    trashold = 20
    y_silent = [x for ind, x in enumerate(signal) if (trashold > x > -trashold)]
    x_silent = [ind for ind, x in enumerate(signal) if (trashold > x > -trashold)]

    print xm_min
    print ym_min
    print 'maxlen'
    print len(_max)
    plot = pylab.plot(signal)
    pylab.hold(True)
    pylab.plot(xm, ym, 'r-')
    pylab.plot(xn, yn, 'g-')
    pylab.plot(xm_min, ym_min, 'm--')
    # pylab.plot(x_silent, y_silent, 'k.')
    pylab.show()